...
def robotControl(trigger, simultaneous_launcher, motor_command_queue, sensor_queue):
    ...
    while not (trigger.is_set() or simultaneous_launcher.broken or gopigo3_robot.volt() <= MINIMUM_VOLTAGE):
        ...
        if command == "stop":
            ...
            move = True
        if command =="forward30cm":
            EasyGoPiGo3().drive_cm(30)
            sleep(5)
        elif command =="left30cm":
            EasyGoPiGo3().turn_degrees(-90)
            EasyGoPiGo3().drive_cm(30)
            sleep(5)
            #gopigo3_robot.left()
        elif command =="right30cm":
            EasyGoPiGo3().turn_degrees(90)
            EasyGoPiGo3().drive_cm(30)
            sleep(5)
        elif command =="back30cm":
            EasyGoPiGo3().turn_degrees(180)
            EasyGoPiGo3().drive_cm(30)
            sleep(5)
        if command == "west":
...
def Main(trigger):
    ...
    available_commands = {"<LEFT>": "west",
                         ...
                         "k":"back30cm",
                         "l":"right30cm",
                         "p":"picture"} # the selectable options within the menu
    menu_order = ["<LEFT>", "<RIGHT>", "<UP>", "<DOWN>", "<SPACE>", "w","i","j","k","l","p"] # and the order of these options
